...
namespace WiringDiagram {
  const int xboxPort = 0;
  const int buttonBoardPort = 1;
  ...
  const int selectbuttonID = 9;
  const int startbuttonID = 10;
  const int joyYID = 1;
  const int solenoidForwardID = 1;
  const int solenoidReverseID = 0;
  const int pneumaticsHubID = 6;
  const int armLimitID = 0;
  const int autoswitchID = 7;
};
